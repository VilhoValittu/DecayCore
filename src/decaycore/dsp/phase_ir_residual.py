    ...
    *,
    ...
    if cfg_reader.bool("enable_residual_pass", False) and cfg_reader.bool("enable_mag_correction", True):
        try:
            ...
            strength = cfg_reader.float("residual_strength", 0.6)
            strength = float(np.clip(strength, 0.0, 1.0))
            mult = cfg_reader.float("residual_smoothing_mult", 2.0)
            ...
            if residual_mode == "off":
                authority_caps = build_residual_authority_caps(freq_axis, residual_pass_mode="off")
            elif not bool(null_guard_enabled):
                authority_caps = _full_residual_authority_caps(
                    freq_axis,
                    max_boost_db=max(float(max_delta_db), authority_max_boost_db),
                    max_cut_db=max(float(max_delta_db), authority_max_cut_db),
                )
            else:
                ...
                if (not np.isfinite(smooth_oct)) or smooth_oct <= 0.0:
                    smooth_oct = 1.0 / 9.0
                authority_null_risk = _smooth_authority_array(
                    _pick_authority_array(st, "authority_null_risk", "null_risk"),
                    freq_axis,
                    smooth_oct,
                    preserve_peaks=True,
                )
                authority_boost = _smooth_authority_array(
                    _pick_authority_array(st, "authority_boost", "boost_authority"),
                    freq_axis,
                    smooth_oct,
                )
                authority_cut = _smooth_authority_array(
                    _pick_authority_array(st, "authority_cut", "cut_authority"),
                    freq_axis,
                    smooth_oct,
                )
                authority_modal_support = _smooth_authority_array(
                    _pick_authority_array(st, "authority_modal_support", "modal_support"),
                    freq_axis,
                    smooth_oct,
                )
                authority_reflection_risk = _smooth_authority_array(
                    _pick_authority_array(st, "authority_reflection_risk", "reflection_risk"),
                    freq_axis,
                    smooth_oct,
                    preserve_peaks=True,
                )
                authority_caps = build_residual_authority_caps(
                    freq_axis,
