...
def _store_phase_excess_diagnostics(
    *,
    ...
    if not isinstance(st, dict):
        return
    try:
        raw_delta = np.asarray(phase_components.raw_u, dtype=float) - np.asarray(
            phase_components.ref_u, dtype=float
        )
...
def _apply_phase_model(  # noqa: C901 - phase containment and spike suppression are kept explicit
    ...
    if use_unified:
        ...
        try:
            if isinstance(st, dict) and unified_gain is not None and np.any(phase_mask):
                ...
                _lf_sel = phase_mask & (f >= 20.0) & (
                    f <= float(getattr(cfg, "low_freq_full_correction_hz", 140.0) or 140.0)
                )
                if np.any(_lf_sel):
                    st["phase_corr_gain_lf_mean"] = float(
                        np.mean(np.asarray(unified_gain, dtype=float)[_lf_sel])
                    )
        except (TypeError, ValueError, FloatingPointError):
            pass
    else:
    ...
    try:
        if isinstance(st, dict):
            st["phase_extra_pre_guard_max_abs_deg"] = _phase_max_abs_deg(
                extra_phase_before_guards, phase_mask
            )
            st["phase_extra_pre_guard_mean_abs_deg"] = _phase_mean_abs_deg(
                extra_phase_before_guards, phase_mask
            )
    except (TypeError, ValueError, FloatingPointError, IndexError):
        pass
                ...
                if use_unified:
                    ...
                    if sel.size >= 4:
                        ...
                        try:
                            ...
                            _sigma_gd = 2.0
                        if gd_ms_arr.size >= 8:
                            gd_ms_arr = scipy.ndimage.gaussian_filter1d(
                                gd_ms_arr, sigma=_sigma_gd, mode="nearest"
                            )
                        if delay_cycles > 0.0:
                        ...
                        if np.any(finite):
                            ratio = limit_ms_arr[finite] / np.maximum(
                                np.abs(gd_ms_arr[finite]), 1e-9
                            )
        ...
        if np.isfinite(max_pre_db):
            ...
            try:
                if isinstance(st, dict):
                    ...
                    st["phase_pre_ringing_before_db"] = (
                        None if pre_before_db is None else float(pre_before_db)
                    )
                    st["phase_pre_ringing_after_db"] = (
    ...
    try:
        if isinstance(st, dict):
            residual_excess = np.asarray(excess_phase, dtype=float) + np.asarray(
                extra_phase, dtype=float
            )
            st["phase_extra_post_guard_max_abs_deg"] = _phase_max_abs_deg(
                extra_phase, phase_mask
            )
            st["phase_extra_post_guard_mean_abs_deg"] = _phase_mean_abs_deg(
                extra_phase, phase_mask
            )
            st["phase_residual_excess_post_guard_max_abs_deg"] = _phase_max_abs_deg(
                residual_excess, phase_mask
            )
            st["phase_residual_excess_post_guard_mean_abs_deg"] = _phase_mean_abs_deg(
                residual_excess, phase_mask
            )
            ...
            if np.isfinite(pre_max) and pre_max > 1e-9 and np.isfinite(post_max):
                st["phase_extra_guard_retained_frac"] = float(
                    np.clip(post_max / max(pre_max, 1e-9), 0.0, 1.0)
                )
    except (TypeError, ValueError, FloatingPointError, IndexError):
        pass
    ...
    if use_unified:
        try:
            ...
            _strength_used = float(st.get("phase_corr_strength_used", float("nan"))) if isinstance(st, dict) else float("nan")
            _gain_lf = float(st.get("phase_corr_gain_lf_mean", float("nan"))) if isinstance(st, dict) else float("nan")
            _fade_lo = float(st.get("mixed_phase_full_correction_hz", float("nan"))) if isinstance(st, dict) else float("nan")
            _fade_hi = float(st.get("mixed_phase_no_correction_hz", float("nan"))) if isinstance(st, dict) else float("nan")
            logger.info(
