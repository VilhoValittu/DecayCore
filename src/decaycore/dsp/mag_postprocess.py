...
def _confpull_post_load_settings(cfg: Any) -> dict[str, float | None | bool]:
    ...
    return {
        ...
        "conf_ceil": conf_ceil,
        "conf_max_hz": conf_max_hz,
        "gamma_cut": gamma_cut,
        "gamma_boost": gamma_boost,
        "conf_sigma": conf_sigma,
        "bass_floor_hz": bass_floor_hz,
        "bass_floor_min": bass_floor_min,
        "bass_boost_floor_hz": bass_boost_floor_hz,
        "bass_boost_floor_min": bass_boost_floor_min,
        "bass_boost_restore": bass_boost_restore,
        "bass_adaptive_isolation_mode": bass_adaptive_isolation_mode,
    }


def _confpull_post_shape_confidence(
    ...
    *,
    conf_sigma: float,
    bass_floor_hz: float,
    bass_floor_min: float,
    bass_boost_floor_hz: float,
    bass_boost_floor_min: float,
) -> np.ndarray:
    ...
    return np.clip(c0, 0.0, 1.0)


def _confpull_post_build_reference(
...
def _confpull_post_apply_bass_restore(
    gain_out: np.ndarray,
    gain_db_in: np.ndarray,
    conf_for_pull: np.ndarray,
    mask_c_in: np.ndarray,
    freq_axis: np.ndarray,
    st: Any,
    *,
    bass_boost_floor_hz: float,
    bass_boost_floor_min: float,
    bass_boost_restore: float,
) -> np.ndarray:
    if bass_boost_floor_hz <= 0.0 or bass_boost_restore <= 0.0:
        return gain_out
    try:
        f0 = np.asarray(freq_axis, dtype=float)
...
def _confpull_post_log_telemetry(
    *,
    gain_db_in: np.ndarray,
    conf_for_pull: np.ndarray,
    mask_c_in: np.ndarray,
    ...
    *,
    ...
    try:
        ...
        if gain_db_in.size < 16 or gain_db_in.shape != mask_c_in.shape or not np.any(mask_c_in):
            return gain_db_in
        settings = _confpull_post_load_settings(cfg)
        conf_floor = float(settings["conf_floor"])
        conf_ceil = float(settings["conf_ceil"])
        conf_max_hz = settings["conf_max_hz"]
        gamma_cut = float(settings["gamma_cut"])
        gamma_boost = float(settings["gamma_boost"])
        conf_sigma = float(settings["conf_sigma"])
        bass_floor_hz = float(settings["bass_floor_hz"])
        bass_floor_min = float(settings["bass_floor_min"])
        bass_boost_floor_hz = float(settings["bass_boost_floor_hz"])
        bass_boost_floor_min = float(settings["bass_boost_floor_min"])
        bass_boost_restore = float(settings["bass_boost_restore"])
        try:
            if isinstance(st, dict):
                st["bass_adaptive_isolation_mode"] = bool(settings["bass_adaptive_isolation_mode"])
                st["conf_pull_post_bass_boost_floor_hz"] = float(bass_boost_floor_hz)
                st["conf_pull_post_bass_boost_floor_min"] = float(bass_boost_floor_min)
                st["conf_pull_post_bass_boost_restore"] = float(bass_boost_restore)
        except (TypeError, ValueError):
            pass
        conf_for_pull = _confpull_post_shape_confidence(
            conf_mask,
            gain_db_in,
            freq_axis,
            conf_sigma=conf_sigma,
        ...
        g_ref = _confpull_post_build_reference(measured_ref_db, gain_db_in, mask_c_in, freq_axis)
        ...
        gain_out = _confpull_post_apply_bass_restore(
            gain_out,
            gain_db_in,
            conf_for_pull,
            mask_c_in,
        ...
        _confpull_post_log_telemetry(
            gain_db_in=gain_db_in,
            conf_for_pull=conf_for_pull,
            mask_c_in=mask_c_in,
            freq_axis=freq_axis,
            tel=tel if isinstance(tel, dict) else None,
            conf_floor=conf_floor,
            conf_ceil=conf_ceil,
            conf_max_hz=conf_max_hz,
            gamma_cut=gamma_cut,
