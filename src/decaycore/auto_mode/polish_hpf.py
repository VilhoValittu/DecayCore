...
def apply_hpf_winner_polish(
    *,
    ...
    def _nearest_allowed_slope(value: float, *, default: int) -> int:
    ...
    def _coerce_freq(value, *, default: float) -> float:
    ...
    def _build_candidate_preset(base_preset: dict, *, enabled_state: bool, freq_hz: float, slope_db_oct: int) -> dict:
    ...
    def _candidate_signature(enabled_state: bool, freq_hz: float, slope_db_oct: int) -> str:
        if not bool(enabled_state):
            return "off"
        return f"on:{float(freq_hz):.1f}:{int(slope_db_oct)}"

    base_meta = dict(base_data.get("_auto_hpf_meta", {}) or {})
    seed_freq_hz = _coerce_freq(
        base_meta.get(
            "freq",
            base_data.get("hpf_freq", 20.0),
        ),
        default=20.0,
    )
    default_slope = int(min(allowed_slopes, key=lambda v: abs(float(v) - 24.0)))
    seed_slope_db_oct = _nearest_allowed_slope(
        base_meta.get(
            "slope_db_oct",
            base_data.get("hpf_slope", default_slope),
        ),
    ...
    current_override = dict(cur_best_preset.get("_auto_hpf_runtime_override", {}) or {})
    current_enabled = bool(
        current_override.get(
            "enabled",
            cur_best_preset.get("hpf_enable", True),
        )
    )
    current_freq_hz = _coerce_freq(
        current_override.get(
            "freq",
            cur_best_preset.get("hpf_freq", seed_freq_hz),
        ),
        default=seed_freq_hz,
    )
    current_slope_db_oct = _nearest_allowed_slope(
        cur_best_preset.get(
            "hpf_slope",
    ...
    seen = {
        _candidate_signature(
            ...
            int(current_slope_db_oct),
        )
    }
    candidate_defs: list[dict] = []
    for cand_enabled, cand_freq_hz, cand_slope_db_oct in candidate_states:
        sig = _candidate_signature(cand_enabled, cand_freq_hz, cand_slope_db_oct)
        if sig in seen:
            continue
        seen.add(sig)
    ...
    if not candidate_defs:
        meta["final_enabled"] = bool(current_enabled)
        meta["final_freq_hz"] = float(current_freq_hz)
        meta["final_slope_db_oct"] = int(current_slope_db_oct)
    ...
    with profiled_section("winner_polish.hpf"):
        ...
        for idx, candidate in enumerate(candidate_defs, start=1):
            cand_test = _build_candidate_preset(
    ...
    final_freq_hz = _coerce_freq(
        final_override.get("freq", cur_best_preset.get("hpf_freq", seed_freq_hz)),
        default=seed_freq_hz,
    )
    final_slope_db_oct = _nearest_allowed_slope(
        cur_best_preset.get(
            "hpf_slope",
