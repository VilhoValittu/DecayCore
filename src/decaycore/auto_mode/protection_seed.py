...
def _shared_stable_crossing_hz(fx: np.ndarray, yx: np.ndarray, thr_db: float) -> float | None:
...
def _auto_mag_c_min_prepare_channel(ff: np.ndarray, mm: np.ndarray, *, default_hz: float) -> dict | None:
...
def _auto_mag_c_min_crossing(prepared: dict | None) -> float | None:
    if not isinstance(prepared, dict):
        return None
    f_lo = np.asarray(prepared.get("ff", np.asarray([], dtype=float)), dtype=float)
    m_env = np.asarray(prepared.get("m_env", np.asarray([], dtype=float)), dtype=float)
    thr_db = float(prepared.get("ref_db", float("nan"))) - 6.0
    above = np.asarray(m_env >= thr_db, dtype=bool)
    if not np.any(above):
        return float(shared._auto_safe_float(prepared.get("default_hz", 25.0), 25.0))
    try:
    ...
    if np.isfinite(y2 - y1) and abs(float(y2 - y1)) > 1e-9:
        return float(x1 + (thr_db - y1) * (x2 - x1) / (y2 - y1))
    return float(x2)


def _auto_hpf_prepare_channel(
    ff: np.ndarray,
    mm: np.ndarray,
    *,
    min_hz: float,
...
def _auto_hpf_estimate_crossings(prepared: dict | None) -> dict | None:
    if not isinstance(prepared, dict):
        return None
    f_lo = np.asarray(prepared.get("f_lo", np.asarray([], dtype=float)), dtype=float)
    m_env = np.asarray(prepared.get("m_env", np.asarray([], dtype=float)), dtype=float)
    ref_db = float(prepared.get("ref_db", float("nan")))
    f3 = _shared_stable_crossing_hz(f_lo, m_env, ref_db - 3.0)
    f6 = _shared_stable_crossing_hz(f_lo, m_env, ref_db - 6.0)
    f12 = _shared_stable_crossing_hz(f_lo, m_env, ref_db - 12.0)
    if f3 is None or f6 is None or not (np.isfinite(f3) and np.isfinite(f6)) or float(f6) >= float(f3):
        return None
    return {
        "f_lo": f_lo,
        "m_env": m_env,
        ...
        "f3": float(f3),
        "f6": float(f6),
        "f12": float(f12) if (f12 is not None and np.isfinite(f12)) else None,
    }


def _auto_hpf_append_secondary_order_estimate(
    n_estimates: list[float],
    *,
    f3: float,
    f12,
    c12: float,
) -> None:
    if f12 is None or not np.isfinite(f12) or float(f12) >= float(f3):
        return
    try:
...
def _auto_hpf_estimate_order(
    prepared: dict,
    *,
    default_slope_db_oct: int,
    ...
    if n_estimates:
        ...
        fit_mask = (
            (f_lo >= shared._auto_safe_float(shared.AUTO_MODE_HPF_MIN_HZ, 20.0))
    ...
    slope_db_oct = int(_floor_slope(6.0 * n_est, allowed_slopes=allowed_slopes, default_slope_db_oct=default_slope_db_oct))
    order = max(1, int(round(float(slope_db_oct) / 6.0)))
    if (f12 is not None and np.isfinite(f12) and float(f12) <= (shared.AUTO_MODE_HPF_MIN_HZ * 1.05)) or float(f6) <= (shared.AUTO_MODE_HPF_MIN_HZ + 1.8):
        slope_db_oct = int(min(int(slope_db_oct), 24))
        order = max(1, int(round(float(slope_db_oct) / 6.0)))
    return int(slope_db_oct), int(order)


def _auto_hpf_estimate_cutoff(
    prepared: dict,
    *,
    order: int,
    min_hz: float,
    ...
    if not fc_candidates:
        return None
    return float(np.clip(float(np.median(np.asarray(fc_candidates, dtype=float))), min_hz, max_hz))


def _auto_hpf_estimate_quality(
    prepared: dict,
    *,
    fc_hz: float,
    order: int,
    ...
    if int(np.count_nonzero(fit_mask)) >= 4:
        ...
        err = np.asarray((fit_mod + delta_db) - fit_meas, dtype=float)
        if err.size >= 8:
...
def _auto_hpf_fit_one_channel(
    ...
    *,
    ...
    prepared = _auto_hpf_prepare_channel(
        ff,
        mm,
        min_hz=min_hz,
        ref_min=ref_min,
    ...
    if prepared is None:
        return None
    crossings = _auto_hpf_estimate_crossings(prepared)
    if crossings is None:
        return None
    slope_db_oct, order = _auto_hpf_estimate_order(
        crossings,
        default_slope_db_oct=default_slope_db_oct,
        allowed_slopes=allowed_slopes,
    )
    fc_hz = _auto_hpf_estimate_cutoff(
        crossings,
        order=order,
        min_hz=min_hz,
        max_hz=max_hz,
    ...
    if fc_hz is None:
        return None
    fit_rmse_db, conf = _auto_hpf_estimate_quality(
        crossings,
        fc_hz=fc_hz,
        order=order,
        min_hz=min_hz,
    ...
    return {
        ...
        "order": int(order),
        "f3_hz": float(crossings["f3"]),
        "f6_hz": float(crossings["f6"]),
        "f12_hz": float(crossings["f12"]) if (crossings.get("f12") is not None and np.isfinite(crossings["f12"])) else float("nan"),
        "fit_rmse_db": float(fit_rmse_db),
        "confidence": float(conf),
    }


def _floor_slope(v: float, *, allowed_slopes: list[int], default_slope_db_oct: int) -> int:
    try:
        x = float(v)
    except (
...
def _nearest_slope(v: float, *, allowed_slopes: list[int], default_slope_db_oct: int) -> int:
    try:
        x = float(v)
    except (
...
def _combine_stereo_hpf(
    ...
    *,
    ...
    if len(valid) == 1:
        ...
        slope_db = int(_nearest_slope(float(v.get("slope_db_oct", default_slope)), allowed_slopes=allowed_slopes, default_slope_db_oct=default_slope_db_oct))
        ...
        slope_db = int(_nearest_slope(slope_w, allowed_slopes=allowed_slopes, default_slope_db_oct=default_slope_db_oct))
        rmse_vals = [
    ...
    return freq_hz, slope_db, conf, fit_rmse_db
...
def _estimate_auto_hpf_from_response(
    ...
    *,
    ...
    fl, ml = _shared_sorted_xy(f_l, m_l)
    fr, mr = _shared_sorted_xy(f_r, m_r)
    ch_l = _auto_hpf_fit_one_channel(
        fl,
        ml,
        ch_name="L",
        min_hz=min_hz,
    ...
    ch_r = _auto_hpf_fit_one_channel(
        fr,
        mr,
        ch_name="R",
        min_hz=min_hz,
    ...
    default_slope = int(_nearest_slope(float(default_slope_db_oct), allowed_slopes=allowed_slopes, default_slope_db_oct=default_slope_db_oct))
    if not valid:
        return {
            "enabled": False,
    ...
    freq_hz, slope_db, conf, fit_rmse_db = _combine_stereo_hpf(
        valid,
        default_freq,
        default_slope,
        min_hz,
