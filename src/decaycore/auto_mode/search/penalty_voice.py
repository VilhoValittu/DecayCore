...
def _corrected_response_from_stats(st: dict | None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    st = dict(st or {})
    freq = _auto_stats_pick_arr(st, "freq_axis")
    measured = _auto_stats_pick_arr(st, "measured_mags")
    filt = _auto_stats_pick_arr(st, "realized_filter_mags", "filter_mags", "predicted_filter_mags")
    ...
    target_use = _aligned_array(target, n, 0.0)
...
def voice_clarity_metrics_from_stats(
    ...
    *,
    ...
    st = dict(st or {})
    freq, corrected, target = _corrected_response_from_stats(st)
    gd = _auto_stats_pick_arr(st, "group_delay_ms", "gd_ms", "gd_curve_ms")
    return compute_voice_clarity_penalty(
        freq,
        corrected,
        target_mag_db=target,
        authority_voice_risk=_auto_stats_pick_arr(st, "authority_voice_risk", "voice_risk"),
        authority_modal_support=_auto_stats_pick_arr(st, "authority_modal_support", "modal_support"),
        authority_null_risk=_auto_stats_pick_arr(st, "authority_null_risk", "null_risk"),
...
def voice_clarity_lr_metrics_from_stats(
    ...
    *,
    ...
    lf, lc, lt = _corrected_response_from_stats(left_st)
    rf, rc, rt = _corrected_response_from_stats(right_st)
