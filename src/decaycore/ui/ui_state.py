...
def _auto_status_detail_init(after: str, low: str, f, r, t) -> str | None:
    if not low.startswith("init"):
        return None
...
def _auto_status_detail_protection(after: str, low: str, f, r, t) -> str | None:
    if not low.startswith("protection seed"):
        return None
...
def _auto_status_detail_target_core(after: str, low: str, f, r, t) -> str | None:
    ...
    if low.startswith("target loaded directly from cache"):
        ...
        if m:
            return f("auto_detail_target_from_cache", target=m.group(1))
    if low.startswith("target preselect top-"):
        ...
        if n_m and candidates:
            ...
            parts = ", ".join(item_tpl.format(name=name, fit=r(fit)) for name, fit in candidates)
            return f("auto_detail_target_preselect_topn", n=n_m.group(1), candidates=parts)
    return None


def _auto_status_detail_target_result_winner(after: str, low: str, f, r, t) -> str | None:
...
def _auto_status_detail_target_result_phase(after: str, low: str, f, r, t) -> str | None:
    ...
    if low.startswith("target shortlist milder skipped"):
        ...
        if m:
            return f("auto_detail_target_shortlist_milder_skipped", a=m.group(1), b=m.group(2))
    if low.startswith("target shortlist"):
        m = re.match(r"target shortlist \(selected (\d+)/(\d+) by spread ([\d.]+) dB\)", after, re.IGNORECASE)
        if m:
            return f("auto_detail_target_shortlist", sel=m.group(1), total=m.group(2), spread=m.group(3))
...
def _auto_status_detail_target_result_misc(after: str, low: str, f, r, t) -> str | None:
    if low.startswith("selecting target curve"):
        ...
        if n_m and leader_m:
            return f("auto_detail_selecting_target", leader=leader_m.group(1), n=n_m.group(1), m=n_m.group(2))
...
def _auto_status_detail_target(after: str, low: str, f, r, t) -> str | None:
    for parser in (
        _auto_status_detail_target_core,
        _auto_status_detail_target_result_winner,
        _auto_status_detail_target_result_phase,
        _auto_status_detail_target_result_misc,
    ):
        parsed = parser(after, low, f, r, t)
        if parsed is not None:
            return parsed
    return None


def _auto_status_detail_refine_phase(after: str, low: str, f, r, t) -> str | None:
    ...
    if low.startswith("cache refine init"):
        ...
        if m:
            return f("auto_detail_cache_refine_init", rounds=m.group(1), trials=m.group(2))
    return None


def _auto_status_detail_refine_cache(after: str, low: str, f, r, t) -> str | None:
    ...
    if low.startswith("cache refine best improved"):
        ...
        if m:
            return f("auto_detail_cache_refine_improved", n=m.group(1), rank=r(m.group(2)))
    return None


def _auto_status_detail_refine_local(after: str, low: str, f, r, t) -> str | None:
...
def _auto_status_detail_refine_misc_local(after: str, low: str, f, r, t) -> str | None:
    if low.startswith("local refine target="):
        ...
        if m2:
            return f("auto_detail_local_refine_target_start_mixed", name=m2.group(1), hz=m2.group(2))
    if low.startswith("local refine center #"):
        ...
        if m:
            return f("auto_detail_local_refine_center_start", n=m.group(1), hz=m.group(2))
    if low.startswith("local refine fallback"):
        m = re.match(r"Local refine fallback center #(\d+)", after, re.IGNORECASE)
        n = m.group(1) if m else "?"
        return f("auto_detail_local_refine_fallback", n=n)
    return None


def _auto_status_detail_refine_misc_reports_a(after: str, low: str, f, r, t) -> str | None:
    if low.startswith("local refine summary"):
        m = re.match(
    ...
    if low.startswith("phase2 summary"):
        ...
        if m:
            return f("auto_detail_phase2_summary", run=m.group(1), ok=m.group(2))
    if low.startswith("phase 3 skipped"):
        return f("auto_detail_phase3_skipped")
    if low.startswith("hpf winner polish improved"):
        m = re.match(r"hpf winner polish improved \(([^,]+), rank ([\d.]+)", after, re.IGNORECASE)
        if m:
            return f("auto_detail_hpf_polish_improved", label=m.group(1).strip(), rank=r(m.group(2)))
    return None
...
def _auto_status_detail_refine_misc_reports_c(after: str, low: str, f, r, t) -> str | None:
    ...
    if low.startswith("micro refine"):
        ...
        if m:
            return f("auto_detail_micro_refine_start", trials=m.group(1))
...
def _auto_status_detail_refine_misc(after: str, low: str, f, r, t) -> str | None:
    for parser in (
        _auto_status_detail_refine_cache,
        _auto_status_detail_refine_local,
        _auto_status_detail_refine_misc_local,
        _auto_status_detail_refine_misc_reports_a,
        _auto_status_detail_refine_misc_reports_c,
        _auto_status_detail_refine_misc_reports_d,
    ):
        parsed = parser(after, low, f, r, t)
        if parsed is not None:
            return parsed
...
def _auto_status_detail_refine(after: str, low: str, f, r, t) -> str | None:
    for parser in (
        _auto_status_detail_refine_phase,
        _auto_status_detail_refine_local,
        _auto_status_detail_refine_misc,
    ):
        parsed = parser(after, low, f, r, t)
        if parsed is not None:
            return parsed
...
def _humanize_auto_status_detail(msg: str) -> str:
    ...
    try:
        for parser in (
            ...
            _auto_status_detail_target,
            _auto_status_detail_refine,
        ):
