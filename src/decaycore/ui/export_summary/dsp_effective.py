...
from .auto_audit_summary import _append_dsp_effective_auto_audit_summary
from .decision_summary import _append_export_decision_summary
from .runtime import _append_main_speaker_xo_hpf_summary
from .winner_polish_summary import (
    _append_dsp_effective_auto_winner_polish_summary,
    _append_dsp_effective_excess_phase_strength_polish_summary,  # noqa: F401
    _append_dsp_effective_hpf_polish_summary,  # noqa: F401
    _append_dsp_effective_low_bass_cut_polish_summary,  # noqa: F401
    _append_dsp_effective_mag_c_min_polish_summary,  # noqa: F401
    _append_dsp_effective_phase_limit_polish_summary,  # noqa: F401
    _append_dsp_effective_residual_peak_polish_summary,  # noqa: F401
    _append_dsp_effective_tdc_strength_polish_summary,  # noqa: F401
)
...
def _append_dsp_effective_auto_tdc_summary(summary_content: str, data, auto_meta: dict, bm: dict, bp: dict) -> str:
    ...
    lines: list[tuple[bool, str]] = [
        (math.isfinite(tdc_sel), f"- Selected TDC strength: {tdc_sel:.1f}%\n"),
        (True, f"- Decay penalty: {tdc_decay_pen:.2f}\n"),
    ]
...
def _append_dsp_effective_auto_modal_summary(summary_content: str, bm: dict) -> str:
    ...
    tdc_modal_reductions = [
        dict(item) for item in list(bm.get("tdc_modal_reductions", []) or []) if isinstance(item, dict)
    ]
...
def _append_dsp_effective_auto_target_summary(summary_content: str, tc: dict) -> str:
    ...
    summary_content += (
        f"Selected target curve: {tc.get('selected_hc_mode', 'n/a')!s} " f"(fit_rms={fit_text}, method={tc_method})\n"
    )
...
def _append_dsp_effective_auto_score_summary(
    ...
    _bass_pen_txt = (
        f", bass_pen={_bass_pen:.2f}" if bool(data.get("bass_integration_enable", False)) and _bass_pen > 0.0 else ""
    )
    ...
    summary_content += f"Automatic quality score: {_display_q:.0f} / 100 " f"({_quality_band(_display_q)})\n"
    summary_content += (
...
def _append_dsp_effective_auto_tracking_summary(summary_content: str, bm: dict) -> str:
    ...
    _rp_sev = _safe_float(
        bm.get("residual_peak_severity", bm.get("worst_residual_peak_db", float("nan"))),
        float("nan"),
    )
...
def _append_dsp_effective_auto_overfit_summary(summary_content: str, bm: dict, residual_peak_polish: dict) -> str:
    ...
    summary_content += (
        ...
        f"  Broad residual peaks: count={int(bm.get('residual_peak_count', 0) or 0)}, worst={_safe_float(bm.get('worst_residual_peak_raw_db', float('nan')), float('nan')):.2f}, "
