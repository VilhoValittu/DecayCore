...
This guide reflects the current DecayCore workflow in v1.0.6: measure with the built-in measurement tool or import existing compatible measurements, keep `AUTO` mode, let DecayCore search for a good preset, export FIR filters, and verify with a new measurement.
...
- In the current v1.0.6 workflow it can also use harmonic curves and IACC-aware ranking to avoid overly aggressive or overly symmetric winners.
